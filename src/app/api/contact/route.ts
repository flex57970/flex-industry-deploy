    ...
    const { error } = await resend.emails.send({
      ...
      html: `<div style="font-family:-apple-system,sans-serif;max-width:600px;margin:0 auto;padding:40px 20px;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="padding:10px 0;color:#71717a;">Nom</td><td>${name}</td></tr>
          <tr><td style="padding:10px 0;color:#71717a;">Email</td><td><a href="mailto:${email}">${email}</a></td></tr>
          ${phone ? `<tr><td style="padding:10px 0;color:#71717a;">Téléphone</td><td>${phone}</td></tr>` : ''}
          ${company ? `<tr><td style="padding:10px 0;color:#71717a;">Entreprise</td><td>${company}</td></tr>` : ''}
          <tr><td style="padding:10px 0;color:#71717a;">Service</td><td>${service}</td></tr>
        </table>
        <div style="margin-top:24px;padding:20px;background:#fafaf9;border-radius:12px;">
          <p style="color:#71717a;margin:0 0 8px;">Message :</p>
          <p style="color:#0a0a0b;white-space:pre-wrap;">${message}</p>
        </div>
      </div>`,
    });
